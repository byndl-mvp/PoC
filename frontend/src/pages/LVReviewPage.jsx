...
const handleGenerateAllQuestions = async () => {
 ...
 const handleDeleteTrade = async (trade) => {
  ...
  const confirmMessage = `Möchten Sie das Gewerk "${trade.name}" wirklich löschen?\n\n` +
    ...
    `• Das generierte Leistungsverzeichnis\n` +
    `• Alle Positionen und Berechnungen\n\n` +
  ...
  if (!window.confirm(`Letzte Bestätigung: "${trade.name}" wirklich unwiderruflich löschen?`)) {
    return;
  }
  
  try {
    // Backend-Request zum Löschen
    const response = await fetch(
      apiUrl(`/api/projects/${projectId}/trades/${trade.id}/delete`),
      {
        method: 'DELETE',
    ...
    // Erfolgsmeldung
    alert(`✅ Gewerk "${trade.name}" wurde erfolgreich gelöscht.`);
  ...
  <div className="flex gap-2">
    ...
    {generatingLVs[trade.id] ? (
      <div className="w-full">
        ...
        {/* NEU: Fake-Ladebalken für LV-Generierung (120 Sekunden) */}
        <div className="mt-2 w-full bg-gray-700/50 rounded-full h-2 overflow-hidden">
          <div 
            className="bg-gradient-to-r from-teal-500 to-blue-600 h-2 rounded-full transition-all duration-1000 ease-out"
            style={{ width: `${lvGenerationProgress[trade.id] || 0}%` }}
          />
        </div>
        <p className="text-xs text-gray-400 mt-1 text-center">
          {Math.round(lvGenerationProgress[trade.id] || 0)}% abgeschlossen
        </p>
      </div>
...
<button
  ...
  disabled={generatingLVs[trade.id]}
  className={`text-sm p-2 rounded-lg transition-all ${
    generatingLVs[trade.id]
      ? 'text-gray-500 cursor-not-allowed opacity-30'
      : 'text-red-400/70 hover:text-red-400 hover:bg-red-500/10'
  }`}
  title={generatingLVs[trade.id] ? 'Löschen während Generierung nicht möglich' : 'Gewerk löschen'}
>
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} 
