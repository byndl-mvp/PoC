    ...
    <div className="space-y-2 sm:space-y-3">
      {lvs.map((lv, idx) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center mt-8 sm:mt-12 max-w-3xl mx-auto">
          ...
          {/* Weiteres Gewerk hinzufügen - kostenpflichtig */}
          <button
            onClick={() => {
              const confirmMessage = projectComplete 
                ? 'Möchten Sie ein weiteres Gewerk hinzufügen?\n\nKosten: 9,90 € für die LV-Erstellung'
                : 'Es sind noch nicht alle Gewerke bearbeitet.\n\nMöchten Sie trotzdem ein zusätzliches Gewerk hinzufügen?\n\nKosten: 9,90 € für die LV-Erstellung';
              
              if (window.confirm(confirmMessage)) {
                handleAddAdditionalTrade();
              }
            }}
            className="flex-1 px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-medium rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all flex flex-col items-center justify-center gap-1"
          >
            ...
            <span className="text-xs text-purple-200 font-normal">9,90 € pro LV</span>
          </button>
