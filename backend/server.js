...
const { CRITICAL_PROMPT_ADDITIONS, validateAndCleanLV } = require('./lv-generator-fix');
...
function validateLvAgainstNeinAntworten(lvData, enrichedIntakeAnswers) {
  console.log('[LV-VALIDATION] Checking for hallucinated positions...');
  
  const errors = [];
  const removedPositions = [];
  
  // ═══════════════════════════════════════════════════════════════════
  // 1. FINDE ALLE "NEIN"-ANTWORTEN
  // ═══════════════════════════════════════════════════════════════════
  
  const neinAntworten = enrichedIntakeAnswers.filter(a => {
    const answer = (a.answer || '').toLowerCase();
    return answer.includes('nein') || 
           answer.includes('vorhandene bleiben') ||
           answer.includes('nicht erneuern') ||
           answer.includes('bleiben wie sie sind');
  });
  
  console.log(`[LV-VALIDATION] Found ${neinAntworten.length} NEIN answers`);
  
  // ═══════════════════════════════════════════════════════════════════
  // 2. DEFINIERE VERBOTENE POSITIONEN BEI BESTIMMTEN "NEIN"-ANTWORTEN
  // ═══════════════════════════════════════════════════════════════════
  
  const verbotenePositionen = [
    {
      frageKeywords: ['fensterbank', 'fensterbrett', 'außenbank'],
      antwortKeywords: ['nein', 'vorhandene bleiben', 'nicht erneuern'],
      positionKeywords: ['fensterbank', 'fensterbrett', 'außenbank', 'innenbank'],
      beschreibung: 'Fensterbänke'
    },
    {
      frageKeywords: ['leibung', 'verputz', 'spachtel'],
      antwortKeywords: ['nein', 'bleiben wie sie sind'],
      positionKeywords: ['leibung', 'verputz', 'spachtel', 'glätten'],
      beschreibung: 'Leibungsarbeiten'
    },
    {
      frageKeywords: ['dämmung', 'isolierung', 'wärmedämmung'],
      antwortKeywords: ['nein', 'nicht erforderlich'],
      positionKeywords: ['dämmung', 'isolierung', 'wdvs'],
      beschreibung: 'Dämmarbeiten'
    },
    {
      frageKeywords: ['rolladen', 'rollladen', 'jalousie'],
      antwortKeywords: ['nein', 'nicht gewünscht'],
      positionKeywords: ['rolladen', 'rollladen', 'jalousie'],
      beschreibung: 'Rollläden'
    },
    {
      frageKeywords: ['insektenschutz', 'fliegengitter'],
      antwortKeywords: ['nein'],
      positionKeywords: ['insektenschutz', 'fliegengitter'],
      beschreibung: 'Insektenschutz'
    },
    {
      frageKeywords: ['gerüst'],
      antwortKeywords: ['nein', 'nicht erforderlich'],
      positionKeywords: ['gerüst'],
      beschreibung: 'Gerüst'
    }
  ];
  
  // ═══════════════════════════════════════════════════════════════════
  // 3. PRÜFE JEDE VERBOTENE KOMBINATION
  // ═══════════════════════════════════════════════════════════════════
  
  verbotenePositionen.forEach(regel => {
    // Finde Antwort zu dieser Frage
    const relevanteAntwort = neinAntworten.find(a => {
      const frage = (a.question || '').toLowerCase();
      return regel.frageKeywords.some(kw => frage.includes(kw));
    });
    
    if (!relevanteAntwort) return; // Keine NEIN-Antwort für diese Frage
    
    // Prüfe ob Antwort wirklich NEIN ist
    const antwort = relevanteAntwort.answer.toLowerCase();
    const istNein = regel.antwortKeywords.some(kw => antwort.includes(kw));
    
    if (!istNein) return;
    
    console.log(`[LV-VALIDATION] Found NEIN for: ${regel.beschreibung}`);
    console.log(`[LV-VALIDATION] Question: ${relevanteAntwort.question}`);
    console.log(`[LV-VALIDATION] Answer: ${relevanteAntwort.answer}`);
    
    // Suche verbotene Positionen im LV
    const verboteneInLv = lvData.positions.filter(pos => {
      const title = (pos.title || '').toLowerCase();
      const desc = (pos.description || '').toLowerCase();
      const text = title + ' ' + desc;
      
      return regel.positionKeywords.some(kw => text.includes(kw));
    });
    
    if (verboteneInLv.length > 0) {
      errors.push({
        regel: regel.beschreibung,
        frage: relevanteAntwort.question,
        antwort: relevanteAntwort.answer,
        positionenGefunden: verboteneInLv.length,
        positionen: verboteneInLv.map(p => p.title)
      });
      
      console.error(`[LV-VALIDATION] 🔴 FEHLER: ${verboteneInLv.length} verbotene Positionen für ${regel.beschreibung}!`);
      verboteneInLv.forEach(p => {
        console.error(`[LV-VALIDATION]   - ${p.pos}: ${p.title}`);
      });
      
      // Entferne diese Positionen
      removedPositions.push(...verboteneInLv);
      lvData.positions = lvData.positions.filter(p => !verboteneInLv.includes(p));
    }
  });
  
  // ═══════════════════════════════════════════════════════════════════
  // 4. PRÜFE MATERIAL-ÜBEREINSTIMMUNG
  // ═══════════════════════════════════════════════════════════════════
  
  const materialAntworten = enrichedIntakeAnswers.filter(a => {
    const frage = (a.question || '').toLowerCase();
    return frage.includes('material') || frage.includes('rahmenmaterial');
  });
  
  materialAntworten.forEach(ma => {
    const antwort = ma.answer.toLowerCase();
    let erwartetesMaterial = null;
    
    if (antwort.includes('holz')) erwartetesMaterial = 'holz';
    else if (antwort.includes('kunststoff')) erwartetesMaterial = 'kunststoff';
    else if (antwort.includes('aluminium') || antwort.includes('alu')) erwartetesMaterial = 'aluminium';
    
    if (!erwartetesMaterial) return;
    
    console.log(`[LV-VALIDATION] Expected material: ${erwartetesMaterial}`);
    
    // Prüfe Positionen
    lvData.positions.forEach(pos => {
      const title = (pos.title || '').toLowerCase();
      const desc = (pos.description || '').toLowerCase();
      const text = title + ' ' + desc;
      
      // Prüfe ob Position Fenster/Türen betrifft
      const istFensterTuer = text.includes('fenster') || text.includes('tür') || text.includes('tuer');
      
      if (istFensterTuer) {
        // Prüfe ob falsches Material
        const hatFalschesMaterial = 
          (erwartetesMaterial === 'holz' && (text.includes('kunststoff') || text.includes('alu'))) ||
          (erwartetesMaterial === 'kunststoff' && (text.includes('holz') || text.includes('alu'))) ||
          (erwartetesMaterial === 'aluminium' && (text.includes('holz') || text.includes('kunststoff')));
        
        if (hatFalschesMaterial) {
          errors.push({
            regel: 'Material-Abweichung',
            position: pos.title,
            erwartet: erwartetesMaterial,
            gefunden: text
          });
          
          console.error(`[LV-VALIDATION] 🔴 MATERIAL-FEHLER: ${pos.title}`);
          console.error(`[LV-VALIDATION]   Erwartet: ${erwartetesMaterial}`);
          console.error(`[LV-VALIDATION]   Gefunden in Position: ${text}`);
        }
      }
    });
  });
  ...
  if (errors.length > 0) {
    console.error('═══════════════════════════════════════════════════════════');
    console.error('🔴🔴🔴 LV-VALIDIERUNG: HALLUZINATIONEN GEFUNDEN 🔴🔴🔴');
    console.error('═══════════════════════════════════════════════════════════');
    console.error(`${errors.length} Fehler gefunden:`);
    errors.forEach((err, idx) => {
      console.error(`\n${idx + 1}. ${err.regel}:`);
      console.error(`   Frage: ${err.frage || 'N/A'}`);
      console.error(`   Antwort: ${err.antwort || 'N/A'}`);
      if (err.positionen) {
        console.error(`   Verbotene Positionen: ${err.positionen.join(', ')}`);
      }
    });
    console.error('═══════════════════════════════════════════════════════════\n');
    
    // Füge Warnung zum LV hinzu
    lvData.validationErrors = lvData.validationErrors || {};
    lvData.validationErrors.halluzinationen = errors;
    
    lvData.additionalNotes = (lvData.additionalNotes || '') + 
      `\n\n⚠️ WARNUNG: LV-Generator hat ${removedPositions.length} nicht bestätigte Positionen erstellt (wurden entfernt).`;
  } else {
    console.log('═══════════════════════════════════════════════════════════');
    console.log('✅ LV-VALIDIERUNG: KEINE HALLUZINATIONEN');
    console.log('═══════════════════════════════════════════════════════════');
    console.log('Alle Positionen entsprechen den Intake-Antworten.');
    console.log('═══════════════════════════════════════════════════════════\n');
  }
